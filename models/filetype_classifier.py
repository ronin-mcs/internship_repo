...
DEFAULT_DATASET_PATH = PROJECT_ROOT / "data" / "byte_chunks_govdocs1_512.npz"
...
class ByteChunkCNN(nn.Module):
    """A compact 1D CNN for byte chunks."""
...
def load_dataset(path: Path):
    ...
    return x, y, label_names, split, split_names


def split_dataset(x: np.ndarray, y: np.ndarray, split: np.ndarray | None, config: TrainConfig):
...
def train_cnn(
    ...
    model = ByteChunkCNN(num_classes=len(label_names)).to(device)
    class_weights = compute_class_weights(y_train, len(label_names), device)
    print("CNN class weights:", [round(float(value), 4) for value in class_weights.cpu()])
    ...
    stale_epochs = 0
    history = []
    best_path = output_dir / "byte_chunk_cnn.pt"
...
def train_random_forest(x_train, x_test, y_train, y_test, label_names, config: TrainConfig, output_dir: Path):
    ...
    model_path = output_dir / "byte_chunk_random_forest.joblib"
    joblib.dump(
        {
            "model": model,
            "label_names": label_names,
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train byte-chunk file type classifiers.")
...
def main() -> None:
    ...
    x, y, label_names, split, split_names = load_dataset(dataset_path)
    x_train, x_val, x_test, y_train, y_val, y_test = split_dataset(x, y, split, config)
    ...
    metrics = {
        ...
        "accuracy_comparison": {
            "cnn": cnn_metrics["test_accuracy"],
            "random_forest": rf_metrics["test_accuracy"],
            "winner": "cnn"
            if cnn_metrics["test_accuracy"] >= rf_metrics["test_accuracy"]
            else "random_forest",
        },
    }

    metrics_path = output_dir / "training_metrics.json"
    cnn_report_csv = output_dir / "cnn_per_class_metrics.csv"
    rf_report_csv = output_dir / "random_forest_per_class_metrics.csv"
    cnn_confusion_csv = output_dir / "cnn_confusion_matrix.csv"
    rf_confusion_csv = output_dir / "random_forest_confusion_matrix.csv"
    cnn_confusion_norm_csv = output_dir / "cnn_confusion_matrix_normalized.csv"
    rf_confusion_norm_csv = output_dir / "random_forest_confusion_matrix_normalized.csv"
    ...
    print("\nFinal comparison")
    print(f"  CNN accuracy:           {cnn_metrics['test_accuracy']:.4f}")
    print(f"  Random Forest accuracy: {rf_metrics['test_accuracy']:.4f}")
    print(f"  Winner: {metrics['accuracy_comparison']['winner']}")
    print_top_k_metrics("CNN", cnn_metrics["top_k_accuracy"])
    print_top_k_metrics("Random Forest", rf_metrics["top_k_accuracy"])
    print_per_class_metrics("CNN", cnn_metrics["classification_report"], label_names)
    print_per_class_metrics("Random Forest", rf_metrics["classification_report"], label_names)
    print_confusion_matrix("CNN", cnn_metrics["confusion_matrix"], label_names)
    print_confusion_matrix("Random Forest", rf_metrics["confusion_matrix"], label_names)
    print(f"Saved metrics to: {metrics_path.resolve()}")
    print(f"Saved CNN per-class CSV to: {cnn_report_csv.resolve()}")
    print(f"Saved Random Forest per-class CSV to: {rf_report_csv.resolve()}")
    print(f"Saved CNN confusion matrix CSV to: {cnn_confusion_csv.resolve()}")
    print(f"Saved Random Forest confusion matrix CSV to: {rf_confusion_csv.resolve()}")
    print(f"Saved CNN normalized confusion matrix CSV to: {cnn_confusion_norm_csv.resolve()}")
    print(f"Saved Random Forest normalized confusion matrix CSV to: {rf_confusion_norm_csv.resolve()}")
